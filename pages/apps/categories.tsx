  ...
  const editData = async (id: any) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${BASE_URL}/marketing/category/${id}`,
        formData,
        {
          maxBodyLength: Infinity,
          headers: {
                ...
                {tableData?.map((data, index) => {
                  return (
                    <tr key={data?._id}>
                      ...
                      <td>
                        <div className="whitespace-nowrap">{data?.icon}</div>
                      </td>
              ...
              <form className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="form-input w-full dark:border-none dark:bg-[#261C16]"
                    name="sub_title"
                    value={categoryData?.sub_title}
                    onChange={handleChange}
                  />
                </div>

                <div>
                  <label htmlFor="ParentCategory" className="dark:text-white">
                    Parent Category
                  </label>
                  <select
                    id="ParentCategory"
                ...
                <div className="flex justify-end">
                  <button
                    type="submit"
                    className="btn btn-primary mt-6 w-fit"
                    onClick={edit ? () => editData(editId) : () => submitData()}
                  >
                    Add Category
                  </button>
                </div>
