...
const KycUser = () => {
    ...
    return loading ? (
        ...
        <div>
                        ...
                        {tableData?.map((data, index) => {
                            return (
                                <tr key={data?._id}>
                                    ...
                                    <a href={`${data.pan_card}`} target="_blank" rel="noopener noreferrer" aria-label="PAN Card">
                                        {data.pan_card}
                                    </a>
                                    <td>
                                        {/* <div className="whitespace-nowrap">{data?.adhar_card_front}</div> */}
                                        <a href={`${data.adhar_card_front}`} target="_blank" rel="noopener noreferrer" aria-label="Aadhar Card Front">
                                            {data.adhar_card_front}
                                        </a>
                                    </td>
                                    <td>
                                        {/* <div className="whitespace-nowrap">{data?.adhar_card_back}</div> */}
                                        <a href={`${data.adhar_card_back}`} target="_blank" rel="noopener noreferrer" aria-label="Aadhar Card Front">
                                            {data.adhar_card_back}
                                        </a>
                                    </td>
                                    <td>
                                        {/* <div className="whitespace-nowrap">{data?.adhar_card_back}</div> */}
                                        <a href={`${data.chequebook}`} target="_blank" rel="noopener noreferrer" aria-label="Aadhar Card Front">
                                            {data.chequebook}
                                        </a>
                                    </td>
                                    <td>
                                        {/* <div className="whitespace-nowrap">{data?.adhar_card_back}</div> */}
                                        <a href={`${data.passbook}`} target="_blank" rel="noopener noreferrer" aria-label="Aadhar Card Front">
                                            {data.passbook}
                                        </a>
                                    </td>
                                    <td>
                                        {/* <div className="whitespace-nowrap"> */}
                                        <select id="Type" className="form-select text-white-dark dark:bg-[#261C16] dark:border-none w-32" onChange={(e) => handleStatus(e, data?._id)} value={data?.status}>
            ...
            {statusModelOpen && <StatusModel setStatusModelOpen={setStatusModelOpen} submitData={submitData} statusId={statusId} status={status} />}
        </div>
    );
}
