  ...
  const updateData = async (id: any) => {
    ...
    try {
      const res = await axios.put(
        `${BASE_URL}/withdraw/updateSetting/${id}`,
        _data,
        {
          maxBodyLength: Infinity,
          headers: {
      ...
      <div className="mb-4 w-full rounded border border-white-light bg-white px-0 shadow-[4px_6px_10px_-3px_#bfc9d4] dark:border-none dark:bg-[#29221C] dark:shadow-custom sm:w-[78.5vw]">
        <div className="p-10">
          <form className="space-y-6">
            <div>
              <label
                htmlFor="default_action_status"
                className="dark:text-white"
              >
                Default Action Status
              </label>
              <select
                id="default_action_status"
                className="form-select text-white-dark dark:border-none dark:bg-[#261C16]"
                name="default_action_status"
                onChange={handleChange}
                value={changeData?.default_action_status}
              >
            ...
            <div>
              <label
                htmlFor="default_external_order_status"
                className="dark:text-white"
              >
                Default External Order Status
              </label>
              <select
                id="default_external_order_status"
                className="form-select text-white-dark dark:border-none dark:bg-[#261C16]"
                onChange={handleChange}
                name="default_external_order_status"
                value={changeData?.default_external_order_status}
              >
                <option value={"OnHold"}>On Hold</option>
                <option value={"InWallet"}>In Wallet</option>
              </select>
            ...
            <div>
              <label
                htmlFor="minimum_withdrawal_amount"
              ...
              <input
                ...
                type="number"
                placeholder="Enter Minimun Withdraw Amount"
                className="form-input w-full dark:border-none dark:bg-[#261C16]"
                onChange={handleChange}
                name="minimum_withdrawal_amount"
                value={changeData?.minimum_withdrawal_amount}
              />
            </div>

            <div>
              <label
                htmlFor="Withdrawal_warning_message"
                className="dark:text-white"
              >
            ...
            <div className="flex justify-end">
              <button
                ...
                onClick={() => updateData(changeData?._id)}
              >
                Submit Setting
              </button>
            </div>
