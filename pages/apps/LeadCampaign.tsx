...
const LeadCampaign = ({ setLeadOpen, rowId }: any) => {
    ...
    const getLead = async () => {
        ...
        console.log(`${BASE_URL}/lead`);
        try {
            const res = await axios.get(`${BASE_URL}/lead/filter?user_id=${user}&start_date=${startDate}&end_date=${endDate}&status=${changeStatus}&campaign_id=${rowId}`, {
                maxBodyLength: Infinity,
                headers: {
                    Authorization: `Bearer ${token}`,
                },
    ...
    return loading ? (
        ...
        <div>
            {leadEditOpen === false && <div className="group relative cursor-pointer overflow-hidden bg-white dark:bg-[#261C16] border-none  ring-1 ring-gray-900/5 dark:ring-gray-700 w-full items-center sm:rounded-lg sm:px-5 ">
                ...
                <div className="xs:px-5 lg:px-0 lg:gap-3 flex lg:flex-row xs:flex-col xs:space-y-4 justify-between my-6">
                    ...
                    <div className="w-full">
                        <label htmlFor="User" className="mt-4">User</label>
                        {/* <input id="User" type="text" placeholder="User" className="form-input h-10 dark:bg-[#1E1611] dark:border-none" /> */}
                        <select id="User" className="form-select text-white-dark h-10 dark:bg-[#1E1611] dark:border-none" onChange={(e) => setUser(e.target.value)} value={user}>
                    ...
                    <div className="w-full">
                        ...
                        <input id="startDate" type="date" placeholder="Start Date" className="form-input h-10 dark:bg-[#1E1611] dark:border-none" onChange={(e) => setStartDate(e.target.value)} value={startDate} />
                    </div>
                    <div className="w-full">
                        <label htmlFor="endDate" >End Date</label>
                        <input id="endDate" type="date" placeholder="End Date" className="form-input h-10 dark:bg-[#1E1611] dark:border-none" onChange={(e) => setEndDate(e.target.value)} value={endDate} />
                    </div>
                    <div className="w-full">
                        <label htmlFor="status" >Status</label>
                        <select id="status" className="form-select text-white-dark h-10 dark:bg-[#1E1611] dark:border-none" onChange={(e) => setChangeStatus(e.target.value)} value={changeStatus}>
                            <option key={"all"} value={"all"}>All</option>
                            {status?.map((option) => (
                                ...
                                {tableData?.map((data: TableData, index) => {
                                    return (
                                        <tr key={data.id}>
                                            ...
                                            <td>{data.name}</td>
                                            <td>{data.name}</td>
                                            <td>{moment(data.created_timestamp).format("DD/MM/YYYY")}</td>
                                            <td>
                                                <span>
                                                    {data.email}
                                                </span>
                                            ...
                                            <td className="text-center">
                                                <div className="dropdown">
                                                    <span className="badge bg-primary cursor-pointer" onClick={() => editClick(data)}>Edit</span>
                                                    <span className="badge bg-danger ml-5 cursor-pointer" onClick={() => deleteLead(data?._id)}>Delete</span>
                                                </div>
                                            </td>
                                        </tr>
                                    );
                        ...
                        <div className="p-8">
                            ...
                            <form className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <input id="Name" type="text" placeholder="Name" className="form-input dark:bg-[#261C16] dark:border-none" name="name" onChange={handleChange} value={changeData?.name} />
                                </div>

                                <div>
                                    <label htmlFor="Email">Email</label>
                                    <input id="Email" type="email" placeholder="Email" className="form-input dark:bg-[#261C16] dark:border-none" name="email" onChange={handleChange} value={changeData?.email} />
                                </div>

                                <div>
                                    <label htmlFor="phoneNumber">Phone Number</label>
                                    <input id="phoneNumber" type="tel" placeholder="Phone Number" className="form-input dark:bg-[#261C16] dark:border-none" name="phone_num" onChange={handleChange} value={changeData?.phone_num} />
                                </div>

                                <div>
                                    <label htmlFor="Status">Status</label>
                                    <select id="Status" className="form-select text-white-dark dark:bg-[#261C16] dark:border-none" name="status" onChange={handleChange} value={changeData?.status}>
                                        <option key={"all"} value={"all"}>All</option>
                                        {status?.map((option) => (
                                            <option key={option} value={option}>
                                                {option}
                                ...
                                <div className="flex justify-end">
                                    <button type="submit" className="btn btn-primary w-fit mt-6" onClick={() => updateLead(rowId)}>
                                        Submit
                                    </button>
                                </div>
                            </form>
